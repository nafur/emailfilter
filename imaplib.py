...
import binascii, errno, random, re, socket, subprocess, sys, time, calendar, select
...
class IMAP4:
    ...
    def idle(self, timeout = 29 * 60):
        ...
        (typ, {data}) = <instance>.idle(timeout)

        Returns dictionary of all untagged responses received while idling.
        """
        if not "IDLE" in self.capabilities:
            raise self.error("server does not support IDLE command.")

        self.untagged_responses = {}
        self.idle_tag = self._command("IDLE")
        self._get_response() # read continuation response
        self.state_before_idle = self.state
        self.state = 'IDLE'
        return 'IDLING', self.untagged_responses

    def idle_poll(self):
        """Checks if there have been any notifications in idle mode since the
        last check. Returns the updates, if there were any.
        
        (typ, {data}) = <instance>.idle_poll()
        """
        assert(self.state == 'IDLE')
        self.untagged_responses = {}
        while True:
            s,foo,bar = select.select([self.sock],[],[],0)
            if len(s) == 0: break
            self._get_response()
        return 'IDLING', self.untagged_responses
    
    def idle_wait():
        """Blocks until something happens on the socket and terminates the
        current IDLE command afterwards.

        (typ, {data}) = <instance>.idle_wait()
        """
        assert(self.state == 'IDLE')
        select.select([self.sock],[],[],timeout)
        return self.idle_done()
    
    def idle_done(self):
        """Terminates the current IDLE command.
        
        (typ, {data}) = <instance>.idle_done()
        
        Returns dictionary of all untagged responses received while idling.
        """
        assert(self.state == 'IDLE')
        self.untagged_responses = {}
        self.send(b"DONE" + CRLF)
        typ,data = self._command_complete("IDLE", self.idle_tag)
        self.state = self.state_before_idle
